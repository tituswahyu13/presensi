...
if (isset($_GET['download_template'])) {
    ...
    // Get employee data (only for 'satpam' role)
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle("Template Jadwal Satpam")
        ->setSubject("Template Import Jadwal")
        ->setDescription("Template untuk import jadwal satpam")
        ->setKeywords("jadwal satpam template excel")
        ->setCategory("Template");
    
    $sheet = $spreadsheet->getActiveSheet();
    $sheet->setTitle('Template Jadwal Satpam');
    ...
    // Output file
    $filename = 'Template_Jadwal_Satpam_' . date('F_Y', strtotime($selected_month . '-01')) . '.xlsx';
            ...
            foreach ($dates as $col => $date) {
                ...
                try {
                    if (!empty($shift)) {
                        ...
                        $check_query = "SELECT * FROM jadwal_satpam WHERE id_pegawai = ? AND tanggal = ?";
                        ...
                        if ($check_result->num_rows > 0) {
                            ...
                            $update_query = "UPDATE jadwal_satpam SET shift = ? WHERE id_pegawai = ? AND tanggal = ?";
                            $update_stmt = $connection->prepare($update_query);
                            $update_stmt->bind_param("sis", $shift, $id_pegawai, $date);
                            $update_stmt->execute();
                        } else {
                            // Insert new schedule
                            $insert_query = "INSERT INTO jadwal_satpam (id_pegawai, tanggal, shift) VALUES (?, ?, ?)";
                        ...
                        $delete_query = "DELETE FROM jadwal_satpam WHERE id_pegawai = ? AND tanggal = ?";
...
// Ambil data pegawai untuk dropdown dan tabel (only for 'satpam' role)
$pegawai_query = "SELECT pegawai.id, pegawai.nama, pegawai.lokasi_presensi 
                  FROM pegawai 
                  LEFT JOIN users ON users.id_pegawai = pegawai.id 
                  WHERE users.role = 'satpam'
...
$jadwal_query = "SELECT js.*, p.nama, p.lokasi_presensi 
                 FROM jadwal_satpam js 
                 LEFT JOIN pegawai p ON js.id_pegawai = p.id 
                 ORDER BY js.tanggal DESC, p.nama ASC";
        ...
        <div class="row mb-3">
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center">
                    <h5>📋 Jadwal Satpam - <?= date('F Y', strtotime($selected_month . '-01')) ?></h5>
                    <div class="d-flex gap-2">
                    ...
                    <div class="card-body">
                        <?php if (empty($pegawai_array)) : ?>
                            <div class="alert alert-warning">
                                ...
                                Pastikan ada karyawan dengan role 'satpam'.
                            </div>
                        <?php else : ?>
                            <div class="mb-3">
