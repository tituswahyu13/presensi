...
$judul = "Rekap Izin/Cuti - Ringkasan";
...
<div class="page-body">
    <div class="container-xl">
        <div class="action-buttons">
            <div class="col-md-12" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <form method="GET" class="date-filter" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                    ...
                    <button type="submit" formaction="rekap_izin_excel.php" class="btn btn-primary export-btn">
                        📊 Export Excel
                    </button>
                </form>
            </div>
        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="filter-buttons">
                    ...
                    <button class="btn btn-primary" onclick="location.href='../presensi/rekap_izin.php'">📈 Izin/Cuti</button>
                </div>
            </div>
        </div>
        ...
        $total_sakit = 0;
        $total_surat_dokter = 0;
        $total_izin = 0;
        $total_cuti = 0;
        $total_dinas_luar = 0;
        $total_work_from_home = 0;
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $total_surat_dokter += $row['jumlah_surat_dokter'];
                $total_izin += $row['jumlah_izin'];
                $total_cuti += $row['jumlah_cuti'];
                $total_dinas_luar += $row['jumlah_dinas_luar'];
                $total_work_from_home += $row['jumlah_work_from_home'];
        ...
        <div class="summary-section">
            ...
            <div class="row">
                ...
                <div class="col-md-2 mb-3">
                    <div class="card border-0 shadow-sm clickable-card" onclick="filterByStatus('surat-dokter')" style="cursor: pointer;">
                        <div class="card-body text-center">
                            <h5 class="card-title text-primary mb-1">📝 Surat Dokter</h5>
                            <h3 class="text-primary mb-0"><?= $total_surat_dokter ?></h3>
                        </div>
                    </div>
                </div>
                <div class="col-md-2 mb-3">
                ...
                <div class="col-md-2 mb-3">
                    <div class="card border-0 shadow-sm clickable-card" onclick="filterByStatus('cuti')" style="cursor: pointer;">
                        <div class="card-body text-center">
                            <h5 class="card-title text-primary mb-1">📅 Cuti</h5>
                            <h3 class="text-primary mb-0"><?= $total_cuti ?></h3>
                        </div>
                    </div>
                </div>
                <div class="col-md-2 mb-3">
                    <div class="card border-0 shadow-sm clickable-card" onclick="filterByStatus('dinas-luar')" style="cursor: pointer;">
                        <div class="card-body text-center">
                            <h5 class="card-title text-primary mb-1">✈️ Dinas Luar</h5>
                            <h3 class="text-primary mb-0"><?= $total_dinas_luar ?></h3>
                        </div>
                    </div>
                </div>
                <div class="col-md-2 mb-3">
                    <div class="card border-0 shadow-sm clickable-card" onclick="filterByStatus('work-from-home')" style="cursor: pointer;">
                        <div class="card-body text-center">
                            <h5 class="card-title text-primary mb-1">🏠 Work From Home</h5>
                            <h3 class="text-primary mb-0"><?= $total_work_from_home ?></h3>
                        </div>
                    </div>
                </div>
                <div class="col-md-2 mb-3">
            ...
            <table class="table table-bordered mt-2">
                <thead>
                    <tr class="text-center">
                        ...
                        <th>Jumlah Sakit</th>
                        <th>Jumlah Surat Dokter</th>
                        <th>Jumlah Cuti</th>
                        <th>Jumlah Dinas Luar</th>
                        <th>Jumlah Work From Home</th>
                        <th>Jumlah Izin</th>
                    ...
                    <?php if (empty($summary_data)) { ?>
                        ...
                        foreach ($summary_data as $rekap) : ?>
                            <tr style="<?= $rekap['lokasi_presensi'] == 'Kantor PDAM' ? 'background-color: #add8e6;' : 'background-color: #d1e7dd;'; ?>"
                                ...
                                data-surat-dokter="<?= $rekap['jumlah_surat_dokter'] ?>"
                                data-izin="<?= $rekap['jumlah_izin'] ?>"
                                data-cuti="<?= $rekap['jumlah_cuti'] ?>"
                                data-dinas-luar="<?= $rekap['jumlah_dinas_luar'] ?>"
                                data-work-from-home="<?= $rekap['jumlah_work_from_home'] ?>"
                                ...
                                <td class="text-center">
                                    <?php if ($rekap['jumlah_surat_dokter'] > 0) : ?>
                                        <span class="badge bg-info"><?= $rekap['jumlah_surat_dokter'] ?></span>
                                    <?php else : ?>
                                        <span class="badge bg-secondary">0</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="text-center">
                                    <?php if ($rekap['jumlah_cuti'] > 0) : ?>
                                        <span class="badge bg-info"><?= $rekap['jumlah_cuti'] ?></span>
                                    <?php else : ?>
                                        <span class="badge bg-secondary">0</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-center">
                                    <?php if ($rekap['jumlah_dinas_luar'] > 0) : ?>
                                        <span class="badge bg-info"><?= $rekap['jumlah_dinas_luar'] ?></span>
                                    <?php else : ?>
                                        <span class="badge bg-secondary">0</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-center">
                                    <?php if ($rekap['jumlah_work_from_home'] > 0) : ?>
                                        <span class="badge bg-info"><?= $rekap['jumlah_work_from_home'] ?></span>
                                    <?php else : ?>
                                        <span class="badge bg-secondary">0</span>
                                    <?php endif; ?>
                                </td>
        ...
        rows.forEach(row => {
            ...
            switch (status) {
                ...
                case 'sakit':
                    ...
                    break;
                case 'surat-dokter':
                    shouldShow = parseInt(row.dataset.suratDokter) > 0;
                    break;
                case 'izin':
                    shouldShow = parseInt(row.dataset.izin) > 0;
                    break;
                case 'cuti':
                    shouldShow = parseInt(row.dataset.cuti) > 0;
                    break;
                case 'dinas-luar':
                    shouldShow = parseInt(row.dataset.dinasLuar) > 0;
                    break;
                case 'work-from-home':
                    shouldShow = parseInt(row.dataset.workFromHome) > 0;
                    break;
