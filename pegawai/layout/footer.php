...
<!-- <div id="closeAppContainer" style="position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%);">
  <button id="closeAppButton" style="
...
<script>
  ...
  // Pastikan tombol "Tutup Aplikasi" hanya ditampilkan pada perangkat mobile
  if (closeAppButton) {
    closeAppButton.addEventListener('click', function() {
      // Konfirmasi sebelum menutup aplikasi
      Swal.fire({
        title: 'Anda yakin?',
        text: "Apakah Anda ingin menutup aplikasi ini?",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          // Cek jika aplikasi berjalan dalam mode standalone
          if (window.matchMedia('(display-mode: standalone)').matches) {
            // Jika aplikasi PWA diinstal dan berjalan dalam mode standalone
            window.close();  // Menutup aplikasi
          } else {
            // Jika tidak dalam mode standalone (buka di browser)
...
// Tambahkan listener untuk menangani navigasi di SPA atau perubahan halaman
window.addEventListener('popstate', function() {
  const closeAppButton = document.getElementById('closeAppButton');
  if (closeAppButton) {
    // Pastikan tombol selalu ada saat pengguna pindah halaman
...
<style>
  ...
  /* Responsif untuk tampilan kecil seperti handphone */
  ...
  /* Responsif untuk tablet dan perangkat lebih besar */
