...
$jam_masuk_gormatted = str_replace(':', '_', $jam_masuk);

$nama_file = 'foto/' . 'masuk_' . $id_pegawai . '_' . $tanggal_masuk . '_' . $jam_masuk_gormatted . '.png';
$file = 'masuk_' . $id_pegawai . '_' . $tanggal_masuk . '_' . $jam_masuk_gormatted . '.png';
...
$stmt_shift = mysqli_prepare($connection, "SELECT masuk_h, pulang_h FROM shift WHERE id = 1");
...
if (!$shift_data || empty($shift_data['masuk_h']) || empty($shift_data['pulang_h'])) {
...
if (mysqli_num_rows($checkResult) > 0) {
    ...
    $stmt_insert = mysqli_prepare($connection, "INSERT INTO presensi(id_pegawai, tanggal_masuk, jam_masuk, foto_masuk, jam_masuk_kantor, jam_pulang_kantor, shift) VALUES (?, NOW(), NOW(), ?, ?, ?, 'H')");
    mysqli_stmt_bind_param($stmt_insert, "isss", $id_pegawai, $file, $shift_data['masuk_h'], $shift_data['pulang_h']);
