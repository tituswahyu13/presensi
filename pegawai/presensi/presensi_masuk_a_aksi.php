...
$stmt_shift = mysqli_prepare($connection, "SELECT masuk_a, pulang_a FROM shift WHERE id = 1");
...
if (!$shift_data || empty($shift_data['masuk_a']) || empty($shift_data['pulang_a'])) {
...
if (mysqli_num_rows($checkResult) > 0) {
    ...
    $stmt_insert = mysqli_prepare($connection, "INSERT INTO presensi(id_pegawai, tanggal_masuk, jam_masuk, foto_masuk, jam_masuk_kantor, jam_pulang_kantor, shift) VALUES (?, NOW(), NOW(), ?, ?, ?, 'A')");
    mysqli_stmt_bind_param($stmt_insert, "isss", $id_pegawai, $file, $shift_data['masuk_a'], $shift_data['pulang_a']);
