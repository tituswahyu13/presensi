...
$stmt_shift = mysqli_prepare($connection, "SELECT masuk_d, pulang_d FROM shift WHERE id = 1");
...
if (!$shift_data || empty($shift_data['masuk_d']) || empty($shift_data['pulang_d'])) {
    $_SESSION['gagal'] = "Jadwal shift tidak lengkap atau tidak ditemukan untuk lokasi Anda.";
    header("Location: ../home/tidar.php"); // Redirect back to the home page
    exit();
}
...
if (mysqli_num_rows($checkResult) > 0) {
    ...
    $stmt_insert = mysqli_prepare($connection, "INSERT INTO presensi(id_pegawai, tanggal_masuk, jam_masuk, foto_masuk, jam_masuk_kantor, jam_pulang_kantor, shift) VALUES (?, NOW(), NOW(), ?, ?, ?, 'D')");
    mysqli_stmt_bind_param($stmt_insert, "isss", $id_pegawai, $file, $shift_data['masuk_d'], $shift_data['pulang_d']);
